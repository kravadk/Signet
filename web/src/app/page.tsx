...
import { listRepos, short } from "@/lib/forge";
    ...
    <div className="wrap">
      <section className="hero stagger">
        ...
        <h1 className="display">
          Forge code into
          <br />
          <span className="molten">verifiable releases.</span>
        </h1>
        <p className="lede">
        ...
        <div style={{ marginTop: 34, display: "flex", gap: 12, flexWrap: "wrap" }}>
          <a className="btn primary" href="#repos">
            Browse repositories
          </a>
          <a
            className="btn"
      ...
      <section className="section" id="repos">
        <div className="section-head">
          <h2>Repositories</h2>
          <span className="faint mono">{repos.length} on testnet</span>
        </div>

        {repos.length === 0 ? (
          <div className="empty">
            No repositories indexed yet. Create one with{" "}
            <span style={{ color: "var(--molten)" }}>forge init</span>.
          </div>
        ) : (
            ...
            {repos.map((r) => (
              <Link key={r.id} href={`/repo/${r.id}`} className="repo-card">
                ...
                <div style={{ marginTop: 16, display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {r.latestRelease ? (
                    <span className="pill release">has release</span>
                  ) : (
                    <span className="pill closed">no release</span>
                  )}
                </div>
                <div className="repo-meta">
                  <span>
                    snapshot <b>{short(r.currentSnapshot, 8, 6)}</b>
                  </span>
                </div>
              </Link>
            ))}
