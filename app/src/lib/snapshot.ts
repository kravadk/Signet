...
// A provenance snapshot anchors the *source tree* a reviewer would review — not
// vendored dependencies, build output, caches or heavy binary assets. Excluding
// those keeps even large repos within the Walrus blob size limit and keeps the
// tree-hash about code, not generated bytes.
const IGNORE = new Set([
  ".git", ".forge", ".sui", ".DS_Store",
  "node_modules", "bower_components", "vendor", "Pods",
  "build", "dist", "out", "target", "bin", "obj",
  ".next", ".nuxt", ".svelte-kit", ".expo", ".vercel", ".netlify", ".angular",
  ".turbo", ".cache", ".parcel-cache", ".gradle", ".dart_tool",
  "coverage", "__pycache__", ".venv", "venv", ".mypy_cache", ".pytest_cache",
]);

// Per-file size cap: skip individual files larger than this (almost always
// bundles/media/binaries, not source). Env-tunable; 0 disables the cap.
const MAX_FILE_BYTES = Number(process.env.SNAPSHOT_MAX_FILE_BYTES ?? 2_000_000);

/** Files skipped by the size cap during the last walk() (surfaced by callers). */
export let lastSkippedLarge: { path: string; size: number }[] = [];
...
function walk(dir: string, root: string, out: string[]): void {
  for (const name of readdirSync(dir)) {
    ...
    if (st.isDirectory()) walk(full, root, out);
      ...
      if (MAX_FILE_BYTES > 0 && st.size > MAX_FILE_BYTES) {
        lastSkippedLarge.push({ path: toPosix(relative(root, full)), size: st.size });
        continue;
      }
      out.push(full);
    }
